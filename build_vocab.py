...
def build_dictionary(texts, vocab_size, lexical, syntactic, semantic):
    ...
    sem_embed = torch.from_numpy(np.array([[float(eval(score,{},{})) for score in line.split()[1:] if bool(re.match(r'^-?\d+(\.\d+)?$', score)) else 0] for line in open(sem_embed_path,'r').readlines()]))
    syn_embed = torch.from_numpy(np.array([[float(eval(score,{},{})) for score in line.split()[1:] if bool(re.match(r'^-?\d+(\.\d+)?$', score)) else 0] for line in open(sem_embed_path,'r').readlines()]))
