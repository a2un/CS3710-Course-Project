...
def build_dictionary(texts, vocab_size, lexical, syntactic, semantic):
    ...
    syn_embed = torch.from_numpy(np.array([[float(eval(score,{},{})) if bool(re.match(r'^-?\d+(\.\d+)?$', score)) else 0 for score in line.split()[1:]] for line in open(syn_embed_path,'r').readlines()]))
